            ...
            {heatmapData.map((item) => (
                ...
                >
                    <div className="flex justify-between items-start mb-2">
                        <h3 className={`font-bold line-clamp-1 ${item.hasData ? 'text-gray-900' : 'text-gray-500'}`} title={item.market.name} style={item.hasData ? { color: getTextColor(item.intensity || 0) } : {}}>
                            {item.market.name}
                        </h3>
                        {item.hasData && (
